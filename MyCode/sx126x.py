...
def receive(self):
        if self.ser.inWaiting() > 0:
            # Chờ dữ liệu load đủ (có thể chỉnh 0.5 nếu thấy 1 hơi chậm)
            time.sleep(1) 
            r_buff = self.ser.read(self.ser.inWaiting())

            # --- 1. In thông tin Header (Người gửi & Tần số) ---
            # r_buff[0], [1] là địa chỉ, [2] là tần số
            sender_addr = (r_buff[0] << 8) + r_buff[1]
            sender_freq = r_buff[2] + self.start_freq
            print(f"Receive from Node: {sender_addr} | Freq: {sender_freq}.125MHz")

            # --- 2. Xử lý nội dung tin nhắn (Logic quan trọng) ---
            msg_content = ""
            
            if self.rssi:
                # TRƯỜNG HỢP 1: RSSI ĐANG BẬT (True)
                # Gói tin có dạng: [Header... Data... RSSI_Byte]
                # Cần cắt bỏ byte cuối cùng ([:-1])
                msg_bytes = r_buff[3:-1]
                
                # Tính toán và in ra độ mạnh sóng
                rssi_val = 256 - r_buff[-1]
                print(f"Packet RSSI: -{rssi_val}dBm")
                
                # Gọi hàm lấy nhiễu môi trường (nếu cần)
                self.get_channel_rssi()
                
            else:
                # TRƯỜNG HỢP 2: RSSI ĐANG TẮT (False)
                # Gói tin có dạng: [Header... Data]
                # Lấy toàn bộ từ byte thứ 3 đến hết ([:])
                msg_bytes = r_buff[3:]

            # --- 3. Giải mã và In ra màn hình ---
            try:
                # Cố gắng chuyển bytes sang chuỗi văn bản (utf-8) để dễ đọc
                msg_content = msg_bytes.decode('utf-8', errors='ignore')
            except:
                # Nếu là dữ liệu nhị phân không đọc được thì để nguyên
                msg_content = str(msg_bytes)

            print(f"Message Content: {msg_content}")
            print("-" * 30) # Kẻ đường gạch ngang cho dễ nhìn
